...
def _layers_dir_path() -> str:
    ...
    """
    try:
        path = _res.files(_pkg).joinpath("latex", "layers")
        # Converte para string e normaliza separadores para LaTeX
        return str(path).replace("\\", "/")
    except Exception:
        # Fallback relativo — útil apenas em dev se os recursos não forem empacotados
        return os.path.join(os.path.dirname(__file__), "latex", "layers").replace("\\", "/")
